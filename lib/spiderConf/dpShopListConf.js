...
var dpShopListConf = {
      ...
      config: [
        ...
        },
        {
          ...
          fn: function ($, item) {
            ...
            var id = parseInt(shopInfo.href.substr(shopInfo.href.indexOf('/shop/') + '/shop/'.length), 10);
            shopInfo.id = id;
            return shopInfo;
          }
        },
        {
          ...
          fn: function ($, item) {
            ...
            var shopComment = {
              star: star / 10,
              commentNums: parseInt($(item).find('.review-num b').text(), 10),
              price: price
            };
  ...
  urlGen: function (response) {
    ...
    if (pageNo === 1 || pageNo <= parseInt(response.maxPageNum, 10)) {
      ...
      var spiderCatNow = category.food.sList[2];
